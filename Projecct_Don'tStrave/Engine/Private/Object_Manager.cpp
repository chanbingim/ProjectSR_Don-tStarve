...
HRESULT CObject_Manager::Remove_Layer(_uint iLevelIndex, const _wstring& strLayerTag)
{
	CLayer* pLayer = Find_Layer(iLevelIndex, strLayerTag);
	if (nullptr == pLayer)
		return E_FAIL;
...
list<CGameObject*>* CObject_Manager::GetAllObejctsToLayer(_uint iLayerLevelIndex, const _wstring& strLayerTag)
{
	CLayer* pLayer = Find_Layer(iLayerLevelIndex, strLayerTag);
	if (nullptr == pLayer)
		return nullptr;

	return pLayer->GetAllObjects();
}
