...
void CFrustomCamera::Compute_CameraMatrix()
{
	...
	for (auto& iter : m_FustomPoints)
	{
		...
		InvPorjection = InvPorjection * m_pTransformCom->Get_World() ;
		_float3	Point = {};
		//투영 행렬의 역행렬을 곱해 view 스페이스 상의 좌표를 얻어낸다.
		D3DXVec3TransformCoord(&Point, &iter, &InvPorjection);
...
_bool CFrustomCamera::IsInObject(const _float3& Point, _float Offset)
{
	...
	D3DXPlaneFromPoints(&m_FustomPlane[1], &m_ComputeFustomPoints[1], &m_ComputeFustomPoints[5], &m_ComputeFustomPoints[6]);
	D3DXPlaneFromPoints(&m_FustomPlane[2], &m_ComputeFustomPoints[3], &m_ComputeFustomPoints[2], &m_ComputeFustomPoints[6]);

	D3DXPlaneFromPoints(&m_FustomPlane[3], &m_ComputeFustomPoints[4], &m_ComputeFustomPoints[0], &m_ComputeFustomPoints[3]);
	D3DXPlaneFromPoints(&m_FustomPlane[4], &m_ComputeFustomPoints[5], &m_ComputeFustomPoints[4], &m_ComputeFustomPoints[7]);
	D3DXPlaneFromPoints(&m_FustomPlane[5], &m_ComputeFustomPoints[4], &m_ComputeFustomPoints[5], &m_ComputeFustomPoints[1]);
