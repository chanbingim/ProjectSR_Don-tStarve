...
CShadowcrawling::CShadowcrawling(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CMonster{ pGraphic_Device }
{

}

CShadowcrawling::CShadowcrawling(const CShadowcrawling& Prototype)
	: CMonster{ Prototype }
{
}

HRESULT CShadowcrawling::Initialize_Prototype()
{
	CXML_Manager::GetInstance()->AddTexture("../Bin/Resources/Textures/Monster/Shadowcrawling/shadowcrawling.scml", L"../Bin/Resources/Textures/Monster/Shadowcrawling/", &m_tImageVec);
	CXML_Manager::GetInstance()->LoadScml("../Bin/Resources/Textures/Monster/Shadowcrawling/shadowcrawling.scml", &m_tAnimation);
	return S_OK;
}

HRESULT CShadowcrawling::Initialize(void* pArg)
{
...
void CShadowcrawling::Update(_float fTimeDelta)
{
...
void CShadowcrawling::Late_Update(_float fTimeDelta)
{
...
HRESULT CShadowcrawling::Render()
{
	__super::Render();
	if (!m_isDead) {
...
void CShadowcrawling::Hit()
{
}

void CShadowcrawling::Attack()
{
	m_bAttack = true;
...
void CShadowcrawling::Death()
{
	if (0 >= m_pMonsterData->iHp) {
		...
		if (GroundObejcts && !GroundObejcts->empty() && 0 < dynamic_cast<CCharacter*>(GroundObejcts->front())->Get_Char()->iHp) {
			...
			player->Get_Player()->iMental = max(0, min(player->Get_Player()->iMaxMental, player->Get_Player()->iMental + 15));
		}
	}
	SetAnimation(DIR::DIR_END, MOTION::DEATH);
}

HRESULT CShadowcrawling::SetAnimation(DIR dir, MOTION motion)
{
	if (DIR::DIR_END == dir) {
		m_tDir = DIR::DOWN;
	}
...
void CShadowcrawling::BeginHitActor(CGameObject* HitActor, _float3& _Dir)
{
}

void CShadowcrawling::OverlapHitActor(CGameObject* HitActor, _float3& _Dir)
{
...
void CShadowcrawling::EndHitActor(CGameObject* HitActor, _float3& _Dir)
{
}

CShadowcrawling* CShadowcrawling::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CShadowcrawling* pInstance = new CShadowcrawling(pGraphic_Device);
...
CGameObject* CShadowcrawling::Clone(void* pArg)
{
	CShadowcrawling* pInstance = new CShadowcrawling(*this);
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CShadowcrawling");
		Safe_Release(pInstance);
	}
